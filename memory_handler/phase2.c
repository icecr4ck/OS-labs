...
int first_fit(MManager *mm, size_t size){
	int offset = -1;
	Block *b = malloc(sizeof(*b));
...
int best_fit(MManager *mm, size_t size){
	int offset = -1;
	int best_size = 0x7fffffff; // plus grand int possible
	Block *b = malloc(sizeof(*b));
	Block *best = malloc(sizeof(*b));
	Block *current = mm->first_block; 
	while (current != NULL){
		if ((current->is_free) && (current->size >= size) && (current->size-size < best_size)){
			best_size = current->size - size;
			best = current;
		}		
		current = current->next;
	}
	if (best_size != 0x7fffffff){
		offset = update_blocks(b, best, size);
		if (best->prev == NULL){
			b->prev = NULL;
			b->next = best;
			mm->first_block = b;
			best->prev = mm->first_block;
		}
		else {
			b->prev = best->prev;
			b->next = best;
...
int worst_fit(MManager *mm, size_t size){
	int offset = -1;
	int worst_size = 0;
	Block *b = malloc(sizeof(*b));
	Block *worst = malloc(sizeof(*b));
	Block *current = mm->first_block; 
	while (current != NULL){
		if ((current->is_free) && (current->size >= size) && (current->size-size > worst_size)){
			worst_size = current->size - size;
			worst = current;
		}		
		current = current->next;
	}
	if (worst_size){
		offset = update_blocks(b, worst, size);
		if (worst->prev == NULL){
			b->prev = NULL;
			b->next = worst;
			mm->first_block = b;
			worst->prev = mm->first_block;
		}
		else {
			b->prev = worst->prev;
			b->next = worst;
...
int next_fit(MManager *mm, size_t size){
	int offset = -1;
	bool round = false;
	Block *b = malloc(sizeof(*b));
	// Commence au dernier bloc alloue
	Block *current = mm->last_alloc; 
	while ((current != mm->last_alloc) || !round){
		// Si le bloc est nul on recommence au head
		if (current == NULL){
			current = mm->first_block;
			round = true;
		}
		if ((current->is_free) && (current->size >= size)){
			offset = update_blocks(b, current, size);
...
int alloumem(MManager *mm, int strategy, size_t size){
	int res = -1;
	switch (strategy){
		case 1:
			res = first_fit(mm,size);
			break;
		case 2:
			res = best_fit(mm,size);
			break;
		case 3:
			res = worst_fit(mm,size);
			break;
		case 4:
			res = next_fit(mm,size);
			break;
		default:
			printf("ERREUR: veuillez sélectionner une stratégie d'allocation valide.\n");
			break;
	}
	clean_memory(mm);
	if (res == -1){
...
int main(int argc, char *argv[]){
	int offset[100] = {0};
	if (argc != 3){
		printf("Syntaxe: ./lab3 memory_size strategy\n");
		exit(1);
	}
	...
	offset[0] = alloumem(mm, strategy, 25);
	offset[1] = alloumem(mm, strategy, 55);
	offset[2] = alloumem(mm, strategy, 5);
	offset[3] = alloumem(mm, strategy, 10);
	offset[4] = alloumem(mm, strategy, 35);
	...
	offset[5] = alloumem(mm, strategy, 15);
	offset[6] = alloumem(mm, strategy, 40);
	...
	offset[7] = alloumem(mm, strategy, 5);
	...
	offset[7] = alloumem(mm, strategy, 20);
